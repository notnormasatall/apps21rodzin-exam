...
public class JSONApp {
    ...
    public static JsonObject sessionResult() {
        ...
        BasicStudent basicStudent = new BasicStudent("Andrii", "Rodionov", 2);
        jsonObject = basicStudent.toJsonObject();

        JsonObject examOne = new JsonObject(new JsonPair("course", new JsonString("OOP")),
                                            new JsonPair("mark", new JsonNumber(3)),
                                            new JsonPair("passed", new JsonBoolean(true)));

        JsonObject examTwo = new JsonObject(new JsonPair("course", new JsonString("English")),
                new JsonPair("mark", new JsonNumber(5)),
                new JsonPair("passed", new JsonBoolean(true)));

        JsonObject examThree = new JsonObject(new JsonPair("course", new JsonString("Math")),
                new JsonPair("mark", new JsonNumber(2)),
                new JsonPair("passed", new JsonBoolean(false)));

        JsonArray exams = new JsonArray(examOne, examTwo, examThree);
